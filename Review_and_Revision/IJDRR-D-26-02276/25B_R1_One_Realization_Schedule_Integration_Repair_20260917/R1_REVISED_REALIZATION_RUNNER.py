...
def main() -> None:
    ...
    output = review / "25B_R1_One_Realization_Schedule_Integration_Repair_20260917"
    output.mkdir(parents=True, exist_ok=True)
    runner_path = Path(__file__).resolve()
    manifest_path = output / "FAILURE_REPAIR_MANIFEST.json"
    ...
    failed_r25 = review / "25_R1_One_Realization_Schedule_Integration_20260917"
    ...
    input_paths = {
        ...
        "parent_failed_manifest": failed_r25 / "ONE_REALIZATION_INPUT_MANIFEST.json",
    }
    ...
    expected_hashes = {
        ...
        "parent_failed_manifest": "a56607dfc43af6460b15446e8c522156c2d2309e06a2ee639d725b15e5ca10b1",
    }
    ...
    parent_manifest = json.loads(input_paths["parent_failed_manifest"].read_text(encoding="utf-8"))
    expected_damage_counts = {"0": 222, "1": 20, "2": 20, "3": 20, "4": 20}
    freeze_checks = {
        "full_r1_id_sha256": newline_hash(full_ids) == parent_manifest["full_r1_id_sha256"],
        "D302_id_sha256": newline_hash(domain_ids) == parent_manifest["D302_id_sha256"],
        "selected_task_ids_sha256": newline_hash(fixture_ledger.R1_station_id.tolist())
        == parent_manifest["selected_task_ids_sha256"],
        "selected_task_ids_exact": fixture_ledger.R1_station_id.tolist()
        == [str(row["R1_station_id"]) for row in parent_manifest["selected_tasks"]],
        "damage_vector_sha256": indexed_numeric_hash(damage)
        == parent_manifest["damage_vector_sha256"],
        "damage_counts": {str(int(k)): int(v) for k, v in damage.value_counts().sort_index().items()}
        == expected_damage_counts == parent_manifest["damage_counts"],
        "duration_vector_sha256": indexed_numeric_hash(duration)
        == parent_manifest["duration_vector_sha256"],
        "hospital_first_sequence_sha256": newline_hash(full_priority)
        == parent_manifest["hospital_first_sequence_sha256"],
        "reference_source_sha256": newline_hash(sorted(REFERENCE_SOURCE_IDS))
        == parent_manifest["source_id_sha256"],
        "functional_threshold": FUNCTIONAL_THRESHOLD == parent_manifest["functional_threshold"],
        "authoritative_mask": parent_manifest["authoritative_mask"]["identified"] == 306
        and parent_manifest["authoritative_mask"]["unidentified"] == 4
        and parent_manifest["authoritative_mask"]["unidentified_ids"] == list(UNRESOLVED_IDS),
        "frozen_input_hashes": all(
            input_hashes_before[key] == value
            for key, value in parent_manifest["input_hashes"].items()
        ),
    }
    if not all(freeze_checks.values()):
        failed = [key for key, passed in freeze_checks.items() if not passed]
        raise RuntimeError(f"Round 25 scientific input freeze mismatch: {failed}")
    ...
    manifest = {
        "schema": "R25B_FAILURE_REPAIR_MANIFEST_V1",
        "scientific_status": "TEST_ONLY_NOT_A_PAPER_RESULT",
        "parent_failed_commit": "bebbae266c659998c5322d91f0f34dc795f2699f",
        "parent_failed_manifest_sha256": input_hashes_before["parent_failed_manifest"],
        "parent_failure_status": "FAIL_REVISED_ONE_REALIZATION_END_TO_END_INTEGRATION_NOT_VERIFIED",
        "exact_scientific_input_equality": all(freeze_checks.values()),
        "scientific_input_equality_checks": freeze_checks,
        "only_code_change": "post-execution QA merge suffix validation and canonicalization",
        "scientific_kernel_unchanged": True,
        "round25b_scientific_execution_count": 1,
        "cumulative_round25_and_round25b_scientific_executions": 2,
    ...
    events = events.merge(fixture_ledger, left_on="task_id", right_on="R1_station_id",
                          ...
                          suffixes=("_schedule", "_fixture")).drop(columns="R1_station_id")
    ...
    if not np.array_equal(events["damage_state_schedule"].to_numpy(),
                          events["damage_state_fixture"].to_numpy()):
        raise RuntimeError("Scheduler damage states differ from frozen fixture")
    if not np.array_equal(events["realized_duration_hr_schedule"].to_numpy(),
                          events["realized_duration_hr_fixture"].to_numpy()):
        raise RuntimeError("Scheduler changed realized durations")
    events["damage_state"] = events["damage_state_schedule"]
    events["realized_duration_hr"] = events["realized_duration_hr_schedule"]
    events = events.drop(columns=[
        "damage_state_schedule", "damage_state_fixture",
        "realized_duration_hr_schedule", "realized_duration_hr_fixture",
    ])
...
- failure-repair manifest SHA-256: `{manifest_hash}`
- parent failed manifest SHA-256: `{input_hashes_before["parent_failed_manifest"]}`
