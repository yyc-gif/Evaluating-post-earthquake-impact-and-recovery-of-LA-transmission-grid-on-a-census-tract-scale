...
The superseded model used 92 nodes and 318 edges. Its retained candidate-facility table contains 486 records, but the historical rule selecting 92 from those 486 could not be recovered. The 486 records are a facility candidate table, not a reconstructed original electrical graph. We therefore cannot provide a retrospective justification for every exclusion in that reduction. Neither that selection nor its graph supplies the revised results.

The revised inventory follows an explicit selection chain from the retained 4,260-record CEC/HIFLD compilation, CA_substations_MERGED.csv: exact TYPE=SUBSTATION leaves 3,374 records; exact STATUS=IN SERVICE leaves 2,775; valid coordinates and a positive original MAX_VOLT of at least 34.5 kV leave 2,548; coverage by the fixed study-area polygon leaves 310. The polygon is the union of the 2,315 retained tract geometries in Tracts_Within_Expanded_Area.csv, including its boundary, rather than a buffer fitted to the selected stations or community results. The voltage cutoff follows the lower bound of the adopted fragility classes. Missing voltage sentinels are not treated as measured positive voltage. Some retained voltage attributes had previously been inferred, as noted in S1.2. TAP, RISER, and DEADEND records do not become substation damage assets, although physical line junctions remain in the geometric representation. Ownership and membership of the largest connected component are not inventory selection criteria. The predeclared boundary-source exception added no records. These are database filtering counts, not a census of the complete Los Angeles electrical system.

Facility adjacency is derived from retained transmission-line geometry projected to EPSG:3310, with retained endpoint/junction snapping, line splitting, and compatible facility registration. A facility pair is linked through a physical line-graph path only when the path contains no intervening registered facility node or its projection anchor; an intermediate station therefore interrupts a direct facility link. Unregistered facility coordinates are not silently treated as registered path blockers. Physical junctions support the paths but are not all counted as facility assets. The retained facility graph has 310 nodes and 1,040 undirected edges, with component sizes 302, 2, 1, 1, 1, 1, 1, and 1. Its underlying collapsed line graph has 68,061 nodes and 69,145 edges. These are different graph representations, neither comparable to the 4,260 database records as an original electrical-network size. Geometric adjacency and registration do not establish switching configuration, line capacity, or verified electrical connectivity.
...
### S5.1 Data versions and retained fields

Source provenance is distinct from output reproducibility. Table S6 separates identifiable data vintages from extraction dates, feature-edit dates, and years that cannot be established from the retained records. The selection history is documented in the retained PROVENANCE_DECISION.md; the source-vintage record is SYSTEM_EPOCH_SERVICE_BOUNDARY_DECISION.md. The NRI data dictionary supplies its explicit version and field definitions. The population field used here is not replaced by the separate population field in NRI, and neither the NRI version nor the year of another Census product establishes the vintage of the retained tract/population file. No current online release was substituted for an experimental input.



Table S6. Retained data versions, fields, and date limitations.
...
Dates refer to the retained experimental inputs. Unknown dates remain unknown; no later online release replaces these data. See S1.1 for inventory selection and S1.3 for access/resource assumptions.



The frozen coverage table preserves the exact tract IDs, population, SOVI, and hospital indicators used in the results. This permits calculation from retained values while leaving the unresolved source dates in Table S6 as provenance limitations. The layers represent different periods, not a synchronized present-day system. Provider terms govern third-party redistribution.
