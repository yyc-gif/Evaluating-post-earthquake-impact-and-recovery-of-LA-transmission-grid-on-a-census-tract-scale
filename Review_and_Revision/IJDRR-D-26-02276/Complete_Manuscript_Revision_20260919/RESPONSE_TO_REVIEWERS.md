...
Response: We removed the broad claim that linking physical damage, operations, and tract consequences is itself a largely unfilled methodological gap. The revised review recognizes Los Angeles restoration scheduling (Çağnan et al., 2006; Xu et al., 2007), access-based resilience (Logan and Guikema, 2020), census-tract power-flow risk assessment (Cheng et al., 2024), and explicit equity-oriented optimization (Toplu-Tutay et al., 2024; Jiang et al., 2025). The narrower contribution is an evaluated comparison showing which burden criteria align or conflict under paired task uncertainty and two stated assumption changes. Locations: Section 1.1, final two paragraphs; Section 1.2; Sections 3–4. We do not claim to be the first study of community consequences or to improve these prior algorithms.
...
Response: The old 92-node/318-edge model no longer supplies the revised results. The current manuscript distinguishes a 310-record retained inventory, 306 state-identified records, a 302-asset primary scheduling component, 196 SCE candidate service nodes, and 817 outcome tracts. The eight excluded task records and reasons are listed in S1.1; all 117 distinct resolved upstream attachment targets are in the task component. We do not imply that 310 is a count of every real Los Angeles electrical asset. Locations: Section 2.1; Table 1; Supplement S1.1. This separates geographic inventory coverage, network representation, repair eligibility, and reported community scope instead of using one reduced count for all four.
