...
# Runtime boundary derived from the official expanded tract whitelist.
CITY_BOUNDARY_SHP: str = str(
    Path(tempfile.gettempdir()) / "la_grid_visualizer_expanded_boundary.geojson"
)


def _find_tract_id_column(columns) -> str:
    """Return the first recognized tract identifier column."""
    preferred = (
        "tract_id",
        "TRACT_ID",
        "GEOID",
    ...
    return next((column for column in preferred if column in columns), list(columns)[0])
...
def _normalize_tract_ids(series: pd.Series) -> pd.Series:
    ...
    normalized = digits.where(digits.notna() & digits.str.len().gt(0)).str.zfill(11)
    return normalized.mask(normalized == "00000000000")


def ensure_study_area_boundary() -> str:
    """Build the map boundary from the official expanded tract whitelist."""
    tracts_path = DATA_DIR / "LA_Tracts_With_Population.shp"
    whitelist_path = DATA_DIR / "Tracts_Within_Expanded_Area.csv"
    if not tracts_path.exists() or not whitelist_path.exists():
        missing = [str(path) for path in (tracts_path, whitelist_path) if not path.exists()]
        raise FileNotFoundError(f"Study-area boundary input missing: {missing}")

    tracts = gpd.read_file(tracts_path)
    whitelist = pd.read_csv(whitelist_path)
    if tracts.empty or whitelist.empty:
        raise ValueError("Study-area boundary inputs must not be empty")

    tracts = tracts.copy()
    tracts["tract_id"] = _normalize_tract_ids(tracts[_find_tract_id_column(tracts.columns)])
    expanded_ids = set(
        _normalize_tract_ids(whitelist[_find_tract_id_column(whitelist.columns)]).dropna()
    )
    selected = tracts[tracts["tract_id"].isin(expanded_ids)].copy()
    if selected.empty:
        raise ValueError("No expanded-area tract IDs matched the tract geometry")

    boundary_geometry = (
        selected.geometry.union_all()
        if hasattr(selected.geometry, "union_all")
        else selected.geometry.unary_union
    )
    ...
    boundary_path = Path(CITY_BOUNDARY_SHP)
    boundary_path.write_text(boundary.to_json(drop_id=True), encoding="utf-8")
...
PANEL_MAP_TALL = {"width_cm": 18.5, "height_cm": 11.8}
PANEL_MAP_STAGE3 = {"width_cm": 13.2, "height_cm": 11.8}
PANEL_GANTT = {"width_cm": 18.5, "height_cm": 9.8}
STAGE3_MAP_FIGURE_ROLE = "PANEL_MAP_STAGE3"
...
FIGURE_SIZE_PRESETS = {
    ...
    "PANEL_MAP_STAGE3": PANEL_MAP_STAGE3,
...
def vis_stage1(gdf):
    ...
    Inputs expected in:  <OUTPUT_ROOT>/Stage 1 Output_expanded/
    ...
    if stage_dir is None:
        print(f"  [Warning] Directory not found: {os.path.join(OUTPUT_ROOT, 'Stage 1 Output_expanded')}")
        return
...
def vis_stage2_topology_with_tracts_latlon():
    ...
    if city.crs is None:
        ...
        if -180 <= minx <= 180 and -90 <= miny <= 90:
            print("   [GIS Warning] CRS missing for City Boundary. Coordinates look like Lat/Lon. Assuming EPSG:4326.")
    ...
    if cec_lines.crs is None:
        ...
        if -180 <= minx <= 180 and -90 <= miny <= 90:
            print("   [GIS Warning] CRS missing for CEC Lines. Coordinates look like Lat/Lon. Assuming EPSG:4326.")
        ...
        try:
            ...
            if city_bnd.crs is None:
                ...
                if -180 <= minx <= 180 and -90 <= miny <= 90:
                    ...
                    print("   [GIS Error] City Boundary CRS missing & huge coords. Cannot assume 3857 safely.")
...
def plot_stage4_crew_base_map(stage4_dir: str) -> None:
    ...
    apply_publication_style()
    fig, ax = plt.subplots(
        figsize=get_figsize("PANEL_FULLROW", width_cm=13.2, height_cm=9.2)
    )
...
def vis_stage4():
    ...
    stage4_dir = os.path.join(OUTPUT_ROOT, "Stage 4 Output_expanded")
...
def vis_stage5():
    ...
    """
    # 1. Locate output directory (Stage 5 Output_expanded or Stage5_Results_expanded)
    stage_dir = os.path.join(OUTPUT_ROOT, "Stage 5 Output_expanded")
    if not os.path.exists(stage_dir):
        stage_dir = os.path.join(OUTPUT_ROOT, "Stage5_Results_expanded")
        if not os.path.exists(stage_dir):
            print("[Warn] Stage 5 output directory not found.")
            return
...
def vis_stage6(
    ...
    if out_dirs and "STAGE6_DIR" in out_dirs:
        ...
        stage_dir = os.path.join(OUTPUT_ROOT, "Stage 6 Output_expanded")
    ...
    # Output goes to Stage 6 Output_expanded folder.
    # ======================================================================
    import glob

    stage3_dir = out_dirs["STAGE3_DIR"] if (out_dirs and "STAGE3_DIR" in out_dirs) else os.path.join(OUTPUT_ROOT, "Stage 3 Output_expanded")
    stage4_dir = out_dirs["STAGE4_DIR"] if (out_dirs and "STAGE4_DIR" in out_dirs) else os.path.join(OUTPUT_ROOT, "Stage 4 Output_expanded")
    stage5_dir = out_dirs["STAGE5_DIR"] if (out_dirs and "STAGE5_DIR" in out_dirs) else os.path.join(OUTPUT_ROOT, "Stage 5 Output_expanded")
    ...
    # Figure 5 uses the 2pc50 topology trajectories. Historical-scenario dual
    # topology panels are not manuscript outputs and are intentionally omitted.
    for scen in ["2pc50"]:
        print(f"\n[Stage 6] Processing Dual Topology: {scen}...")

        required_topology_files = [
            os.path.join(stage3_dir, f"graph_robustness_mean_{scen}.csv"),
            *[
                os.path.join(stage4_dir, f"rule_graphrobustness_{scen}_{rule}.csv")
                for rule in RULES
            ],
            *[
                os.path.join(stage5_dir, f"ga_graphrobustness_{scen}_{policy}.csv")
                for policy in ("Balanced", "HospFirst", "Efficiency")
            ],
        ]
        missing_topology_files = [
            path for path in required_topology_files if not os.path.exists(path)
        ]
        if missing_topology_files:
            raise FileNotFoundError(
                "Figure 5 topology inputs are incomplete: "
                + ", ".join(missing_topology_files)
            )
...
def vis_stage7_cluster_top10_impact_degree_km(
    ...
    """
    stage7_dir = os.path.join(OUTPUT_ROOT, "Stage 7 Output_expanded")
    ...
    stage2_dir = os.path.join(OUTPUT_ROOT, "Stage 2 Output_expanded")
...
# ==============================================================================
def main() -> None:
    """Generate all manuscript-stage figures for the official study area."""
    global CITY_BOUNDARY_SHP
    if hasattr(sys.stdout, "reconfigure"):
        sys.stdout.reconfigure(encoding="utf-8", errors="replace")
    if hasattr(sys.stderr, "reconfigure"):
        sys.stderr.reconfigure(encoding="utf-8", errors="replace")
    CITY_BOUNDARY_SHP = ensure_study_area_boundary()

    print("=========================================")
    print(" MANUSCRIPT VISUALIZATION GENERATOR")
    print("=========================================")
    print("Loading study-area tract geometry...")
    gdf_study_area = load_shapefile()

    vis_stage1(gdf_study_area)
    vis_stage2()
    vis_stage3(gdf_study_area)
    vis_stage4()
    vis_stage5()
    vis_stage6()
    vis_stage7_cluster_top10_impact_degree_km(
        gdf_study_area,
        cluster_id=1,
        top_n=10,
        impact_mode="lambda2",
    )
    vis_stage7(gdf_study_area)
    print("\nManuscript visualizations completed.")
